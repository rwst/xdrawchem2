...
{
    QList < Molecule * >reactantList;
    QList < Molecule * >productList;
    ...
    foreach ( QSharedPointer<Drawable> tmp_draw, drawlist ) {
        if ( tmp_draw->metaObject() == &Molecule::staticMetaObject ) {
            ...
            if ( tmp_mol->groupType() == GROUP_PRODUCT )
                productList.append( tmp_mol );
        }
    }

    if ( ( reactantList.count() == 0 ) || ( productList.count() == 0 ) ) {
        QMessageBox::information( 0, tr( "Missing reactant or product" ), tr( "You must select at least one reactant and one product." ) );
        return;
    }

    qDebug() << "Reaction analysis called";

    if ( flags == RXN_ENTHALPY_ESTIMATE ) {
        qDebug() << "Enthalpy change";
...
//        double shift1;
        ...
        if ( ( reactantList.count() > 1 ) || ( productList.count() > 1 ) ) {
            QMessageBox::warning( 0, tr( "1H NMR compare error" ), tr( "Please select only one reactant and one product to compare." ) );
            return;
        }
    ...
    if ( flags == RXN_13CNMR ) {
        if ( ( reactantList.count() > 1 ) || ( productList.count() > 1 ) ) {
            QMessageBox::warning( 0, tr( "13C NMR compare error" ), tr( "Please select only one reactant and one product to compare." ) );
            return;
        }
