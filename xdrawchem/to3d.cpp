...
void Molecule::Make3DVersion( QString fn3d )
{
    ...
    foreach ( DPoint *tpt, up ) {
        tpt->serial = i;
        i++;                    // serialize
        tpt->new_order = 0;  // we'll use this as a status flag
        tpt->hit = false;
    }
    ...
    foreach ( DPoint *tpt, up ) {
        if ( tpt->getAtomicNumber() == 6 ) {
            if ( tpt->neighbors.count() == 4 ) {
                // NO chirality.
                tout << tpt->baseElement() << " -1 1 ";
            } else {
                // Consider chirality.
                tout << tpt->baseElement() << " 0 1 ";
            }
        } else {
            tout << tpt->baseElement() << " -1 1 ";
        }
        foreach ( n_pt, tpt->neighbors ) {
            tout << n_pt->serial << " ";
        }
        tout << "Z";
    }
    ...
    if ( fn3d.length() > 1 ) {
        ...
        fd.setFilters( QStringList( tr( "MDL molfile (*.mol)" ) ) );
