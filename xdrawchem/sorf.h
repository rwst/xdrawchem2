  ...
  */
  setOfRings findSSSR(QVector<Atom *> atSet){
    ...
    */
	...
	for (f = 0; f < nodesN2.count(); f++){
	  ...
	  if (ring->size() > 0){
	    ...
	    if (!sssr.ringAlreadyInSet(ring)){
	      ...
	      remembernodes.insert(nodesToBreakCounter, nodesN2.at(f));
	      nodesToBreakCounter ++;
	    }
	  }
	}
    ...
    qDebug() << "SSSR:" << sssr.size() ;
    for (int h = 0; h < sssr.size(); h++)
      qDebug() << h << ":" << sssr[h]->size() ;
    qDebug() << "SSSR ends" ;
    return sssr;
  }
...
//    Ring *ringset;
	...
	FGsource.insert(neighbor->number, rootNode);
	...
	if (m->degree > 0 && m != source){
	  if (path[m->number]->count() > 0){
		...
		//System.out.println("Ring closure found: " + m.number + ", source is " + source.number);
		//System.out.println(m + ", " + source);

		//System.out.println("Path of frontnode: " + path[frontNode.number].toString());
	    ...
	    // update the path[m]
	    path.insert(m->number, merge(path[m->number], path[frontNode->number]));
	    path[m->number]->insert(0, m);
	    // now push the node m onto the queue
	    FGsource.insert(m->number, frontNode);
	    queue.enqueue(m);
	  }
	}
      }
